...
import type { MemoryDetailJson } from "@/components/memories/memories-client";
import { useMemoriesClient } from "@/components/memories/memories-client-provider";

export function useMemoryDetail(input: {
  namespace: string;
  key: string;
  open: boolean;
  rootHex?: string;
  ...
  const entityKey = `${input.namespace}:${input.key}`;
  const [detailEntityKey, setDetailEntityKey] = useState(entityKey);
  const [detail, setDetail] = useState<MemoryDetailJson | null>(null);
  ...
  useEffect(() => {
    ...
    return () => ac.abort();
  }, [input.open, input.namespace, input.key, input.includeVectors, queryRootHex, client]);
